            ...
            with transaction.atomic():
                for code, name, description, unit_code, unit_name, warehouse_code, warehouse_name, price, cnt in reader:
                    ...
                    try:
                        ...
                        house = Warehouse.objects.create(code=warehouse_code,
                                                         name=force_text(warehouse_name.decode('gbk')))

                    try:
                        material = Material.objects.get(code=code)
                    except Exception, e:
                        material = Material(code=code, name=force_text(name.decode('gbk')),
                                            spec=force_text(description.decode('gbk')), warehouse=house)
                        material.stock_price = price
                        material.save()
                    InitItem.objects.create(master=self, material=material, price=price, cnt=cnt, warehouse=house,
                                            measure=measure, source=self.code)
        ...
        if count > 0:
            with transaction.atomic():
                ...
                for item in InitItem.objects.filter(master=self).all():
                    Inventory.objects.create(material=item.material, price=item.price, cnt=item.cnt,
                                             warehouse=item.warehouse, measure=item.measure)
                    item.status = 1
...
class StockIn(generic.BO):
    ...
    po = models.ForeignKey(PurchaseOrder, verbose_name=_("purchase order"), null=True,
                           limit_choices_to={"entry_status": "0"}, blank=True)
        ...
        if self.initem_set.count() > 0:
            with transaction.atomic():
                ...
                for item in self.initem_set.filter(status=0).all():
                    try:
                        inventory = Inventory.objects.get(warehouse=self.warehouse, material=item.material,
    ...
    def save(self, force_insert=False, force_update=False, using=None,
        ...
        if self.out and item_count == 0:
            for out_item in OutItem.objects.filter(master=self.out):
                ReturnItem.objects.create(master=self, out_item=out_item, material=out_item.material,
                                          price=out_item.price,
                                          measure=out_item.measure, warehouse=out_item.warehouse, out_cnt=out_item.cnt,
                                          cnt=out_item.cnt)
        ...
        with transaction.atomic():
            ...
            for item in ReturnItem.objects.filter(master=self):
                if item.cnt > item.out_item.cnt or item.cnt < 0:
                    raise Exception(
                        '%s cnt is invalid,out is %s,return is %s' % (item.material, item.out_cnt, item.cnt))
                item.event_time = datetime.datetime.now()
                item.source = self.code
                item.status = 1
...
class InItem(InOutDetail):
    ...
    def get_new_price(self):
        if self.po_item and self.master.warehouse:
            try:
                inventory = Inventory.objects.get(warehouse=self.master.warehouse, material=self.material,
