...
class SaleOrder(generic.BO):
    ...
    amount = models.DecimalField(_("money amount"), max_digits=12, decimal_places=2, blank=True, null=True,
                                 default=0.00)
    ...
    def collection_amount(self):
        return PaymentCollection.objects.filter(so=self).aggregate(Sum('collection_amount')).get(
            'collection_amount__sum') or 0.00
...
class SaleItem(models.Model):
    ...
    def save(self, force_insert=False, force_update=False, using=None,
        ...
        if self.material.sale_price or self.material.stock_price:
            self.sale_price = self.material.sale_price or self.material.stock_price * decimal.Decimal(
                1.17) / decimal.Decimal(0.6)
            ...
            with transaction.atomic():
                ...
                for row_index in range(row_count):
                    ...
                    try:
                        ...
                        material.save()
                    OfferItem.objects.create(master=self, material=material, measure=measure, cnt=row[8],
